...
`Call`: Owns local media, `SignalingChannel`, and the active `PeerConnectionClient`. Both mode transitions are make-before-break and symmetric: the outgoing client is retained for media continuity while the incoming one negotiates with the same local tracks. During P2P→SFU upgrade it retains `p2pPcClient_` and closes it when the SFU transport's ICE connects. On an `sfu-downgrade` control it retains `sfuPcClient_` (silencing its callbacks first, so a late event from the retired transport cannot act on the new client) and builds a direct P2P client, with the control-elected `is_initiator` making one side the offerer; here ICE connecting only commits the mode, and `AppController` closes the retained client through `releaseRetiredSfuTransport()` once its frames are off screen. A bounded timer completes the downgrade even if the peer never negotiates.
...
the version and variant stamped in) and renders as 22 base64url characters, so V2 shows the room link as a real anchor —
clickable, hoverable, openable in a new tab — in place of the text field, and the button reads
GENERATE; V1 keeps its free-form numeric room id and RANDOM. A plain click joins in-app via
`loadRoom_`; modified clicks are left to the browser. `isRoomToken` mirrors the Rust validator
