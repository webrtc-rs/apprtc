...
```bash
sudo mkdir -p /opt/apprtc
sudo rsync -avz --exclude target --exclude .git --exclude .idea ./ root@SERVER:/opt/apprtc/
```
...
sudo install -m 0755 target/release/apprtc /usr/local/bin/apprtc
```

## 6. Configure systemd

Create `/etc/systemd/system/apprtc.service`:
...
ExecStart=/usr/local/bin/apprtc --host appr.tc --port 443 --web-root /opt/apprtc/appweb --tls --certificate /etc/letsencrypt/live/appr.tc/fullchain.pem --private-key /etc/letsencrypt/live/appr.tc/privkey.pem --level info
Restart=always
RestartSec=5
KillSignal=SIGINT
TimeoutStopSec=30
...
Run `/usr/local/bin/apprtc --help` for the authoritative option list.
