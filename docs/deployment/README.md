...
AppWeb receives the public browser URL through `--signaling-url`. Its private room-authority traffic independently uses `--signaling-grpc-url https://appr.tc:50051`; no `/app` WebSocket endpoint is exposed.
...
Run `appweb --help` and `signaling --help` for the authoritative options. Both support `--host-ip`, `--port`, `--tls`, `--certificate`, `--private-key`, `--debug`, `--level`, and `--output-log-file`. AppWeb additionally supports `--public-url`, `--signaling-url`, `--signaling-grpc-url`, `--signaling-token`, `--signaling-insecure-tls`, `--web-root`, ICE options, banner configuration, and `--bypass-join-confirmation`. Signaling additionally supports `--grpc-port` and `--grpc-token` for its private service API. Its `--host-ip` applies to both listeners, and its `--tls` flag protects both listeners with the same certificate.
