...
cargo run -p apprtc --bin signaling -- --host-ip 127.0.0.1 --port 8081 \
  --grpc-port 50051 --tls -d -l trace -o ~/Downloads/signaling.log &
...
cargo run -p apprtc --bin sfu -- --host-ip 127.0.0.1 --public-ip 127.0.0.1 \
  ...
  --grpc-url https://127.0.0.1:50051 --insecure-tls -d -l trace -o ~/Downloads/sfu.log &
...
cargo run -p apprtc --bin appweb -- --host-ip 127.0.0.1 --port 8080 --web-root appweb \
  ...
  --grpc-url https://127.0.0.1:50051 --insecure-tls --tls -d -l trace -o ~/Downloads/appweb.log &
