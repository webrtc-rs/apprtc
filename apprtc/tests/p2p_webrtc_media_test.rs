...
struct Events {
    ...
    tracks: mpsc::UnboundedSender<Arc<dyn webrtc::media_stream::track_remote::TrackRemote>>,
}
...
type PeerParts = (
    ...
    mpsc::UnboundedReceiver<Arc<dyn webrtc::media_stream::track_remote::TrackRemote>>,
);
...
async fn peer() -> Result<PeerParts> {
    ...
    let pc = PeerConnectionBuilder::new()
        ...
        .build()
        .await?;
    Ok((Arc::new(pc), states_rx, out_tx, out_rx, tracks_rx))
}
...
async fn completes_v1_signaling_and_webrtc_datachannel_connection() -> Result<()> {
    ...
    let first_id = first["params"]["client_id"]
        ...
        .to_owned();
    let (initiator_pc, mut initiator_states, _initiator_out, initiator_out_rx, _initiator_tracks) =
        peer().await?;
    ...
    let (callee_pc, mut callee_states, callee_out, callee_out_rx, mut callee_tracks) =
        peer().await?;
    ...
    // Send a small sequence of valid Opus RTP packets so the negotiated remote
    // track is actually instantiated by the callee.
    for sequence_number in 0..20u16 {
        track
            .write_rtp(Packet {
    ...
    // The negotiated audio m-line must surface as a remote track, proving that
    // media transceivers survive the same V1 P2P signaling path as the data channel.
    timeout(CONNECT_TIMEOUT, callee_tracks.recv())
        .await
        .context("timed out waiting for negotiated remote audio track")?
        .ok_or_else(|| anyhow!("remote track channel closed"))?;
